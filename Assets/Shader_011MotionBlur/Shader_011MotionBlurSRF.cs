...
public class Shader_011MotionBlurSRF : ScriptableRendererFeature
{
    class Shader_011MotionBlurPass : ScriptableRenderPass
    {

        public static string k_RenderTag = "Shader_011MotionBlurPass";

        private Shader_011MotionBlurVolume volume;
        ...
        private static readonly int previousView2WorldMat = Shader.PropertyToID("previousView2WorldMat");
        
        // private RenderTextureDescriptor _cameraTextureDescriptor;

        private Matrix4x4 previousView2World;

        public Shader_011MotionBlurPass()
        {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            volume = stack.GetComponent<Shader_011MotionBlurVolume>();
            if (!volume.IsActive())
            {
                return;
            }
            ...
            _material.SetVector(_blurSize,volume._blurSize.value);

            _material.SetMatrix(previousView2WorldMat,previousView2World);
            MaterialEnableKey("CAMERA_VERTEX", volume.intensity.value == 1);
            MaterialEnableKey("PRE_CAMERA_VERTEX", volume.intensity.value == 2);

            MaterialEnableKey("CLIP", volume.intensity.value == 3);
            MaterialEnableKey("PRO_CLIP", volume.intensity.value == 4);
            ...
            previousView2World = worldToCameraMatrix.inverse;
        }
        ...
        {
            string shaderName = "Shader/Shader_011MotionBlur";
            return shaderName;
        }
    ...
    Shader_011MotionBlurPass m_ScriptablePass;

    /// <inheritdoc/>
    public override void Create()
    {
        m_ScriptablePass = new Shader_011MotionBlurPass();
